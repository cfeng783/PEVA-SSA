...
public class Epidemics {
	...
	public static void main(String[] args) {
		ArrayList<Loc> locs = TopologyFactory.makeTopology(TopologyFactory.GRID, 10);
		for(Loc loc: locs) {
			loc.print();
		}
				...
				if(data.equalsIgnoreCase("#states")){
					...
					for(int i=0; i<locs.size(); i++) {
						...
						String susceptible = st_s + loc.toString() + ":= ?("
								+ msg_contact + "," + p + ")@Pr{1/(|" + st_s + loc.toString() + "|+|" + st_i + loc.toString() + "|+|"
								//		+ st_r + comCod(i) + "|+0.0001)}." + st_i + comCod(i);
								+ st_r + loc.toString() + "|)}." + st_i + loc.toString();
