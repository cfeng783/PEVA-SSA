    ...
    public void BellmanFordEvaluation(int source, int adjacencymatrix[][])
    {
        ...
        for (int node = 1; node <= numberofvertices - 1; node++)
        {
            for (int sourcenode = 1; sourcenode <= numberofvertices; sourcenode++)
            {
                ...
                {
                    if (adjacencymatrix[sourcenode][destinationnode] != MIN_VALUE)
                    {
...
class DijkstraShortesPath
{
    ...
    public int getNodeWithMinimumDistanceFromUnsettled(boolean unsettled[])
    {
        ...
        for (int vertex = 1; vertex <= numberofvertices; vertex++)
        {
            if (unsettled[vertex] == true && distances[vertex] > max)
            {
                node = vertex;
    ...
    public void evaluateNeighbours(int evaluationNode)
    {
        ...
        for (int destinationNode = 1; destinationNode <= numberofvertices; destinationNode++)
        {
            ...
            {
                if (adjacencymatrix[evaluationNode][destinationNode] != MIN_VALUE)
                {
